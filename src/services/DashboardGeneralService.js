...
async function fetchReal(params = {}) {
  const { eventoId, fechaInicio, fechaFin } = params;
  
  // Base URL
  let url = "/eventos/dashboard";
  
  // Query Parameters
  const queryParams = new URLSearchParams();
  if (fechaInicio) queryParams.append("fecha_inicio", fechaInicio);
  if (fechaFin) queryParams.append("fecha_fin", fechaFin);
  if (eventoId) queryParams.append("evento_id", eventoId);
  ...
  // console.log(payload.estadisticas, "-------------------------->>>");
  return {
...
const DashboardGeneralService = {
  /**
   * Obtener resumen dashboard de un evento
   * @param {object} params { eventoId, fechaInicio, fechaFin }
   * @param {object} opts { forceMock?: boolean }
   */
  async getResumen(params = {}, opts = {}) {
    if (opts.forceMock) return fetchMock();
    try {
      return await fetchReal(params);
    } catch (e) {
      if (process.env.NODE_ENV !== "production") {
        console.warn(
          "[DashboardGeneralService] fallo endpoint real, usando mock:",
...
export default DashboardGeneralService;
