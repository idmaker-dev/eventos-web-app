...
export default function MonitordeChats() {
  ...
  const conversacionesIniciales = {
    1: [
      ...
      { id: 2, remitente: "yo", texto: "¡Hola María! Tu ceremonia de graduación está programada para el 15 de junio de 2025 a las 6:00 PM. 👍", tiempo: "Hace 5 minutos" },
      { id: 3, remitente: "contacto", texto: "¿Dónde será el evento?", tiempo: "Hace 4 minutos" },
      { id: 4, remitente: "yo", texto: "El evento se llevará a cabo en el Auditorio Principal de la Universidad, ubicado en Av. Central #123. 😀", tiempo: "Hace 2 minutos" },
    ],
    2: [
  ...
  // Enviar mensaje (tú = gris, derecha)
  const enviarMensaje = () => {
        ...
        <div className="flex-1 p-4 space-y-4 overflow-y-auto bg-gray-100">
          {(conversaciones[chatSeleccionado] || []).map((m) => (
            <div
              key={m.id}
              className={`flex ${
                m.remitente === "yo" ? "justify-end" : "justify-start"
              }`}
            >
              <div
                className={`max-w-xs md:max-w-md px-4 py-2 rounded-lg shadow-sm text-sm ${
                  m.remitente === "yo"
                    ? "bg-gray-200 text-gray-800" // tus mensajes = gris
                    : "bg-green-100 text-gray-900" // contacto = verde
                }`}
              >
                <p>{m.texto}</p>
