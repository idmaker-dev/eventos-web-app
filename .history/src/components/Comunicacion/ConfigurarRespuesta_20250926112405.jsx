...
export default function ConfigurarRespuesta() {
  ...
  // 🔹 Cargar preguntas y abiertos desde localStorage al iniciar
  useEffect(() => {
    const dataPreguntas = localStorage.getItem("preguntas");
    const dataAbiertos = localStorage.getItem("abiertos");

    if (dataPreguntas) setPreguntas(JSON.parse(dataPreguntas));
    if (dataAbiertos) setAbiertos(JSON.parse(dataAbiertos));
  }, []);

  // 🔹 Guardar preguntas cada vez que cambien
  useEffect(() => {
